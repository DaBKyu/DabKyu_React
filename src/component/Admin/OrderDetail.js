...
const OrderDetail = () => {
  ...
  const [usedCouponSeqno, setUsedCouponSeqno] = useState(null); 
  ...
  const cancelOrder = async () => {
    // 첫 번째 주문 정보 확인
    const orderDetailSeqno = orderDetails[0].orderDetailSeqno;  
    console.log(orderDetailSeqno);
    const usedCouponSeqno = orderInfo.usedCouponSeqno; 
    const usedPoint = orderInfo.usedPoint; 
  
    try {
      const response = await fetch('http://localhost:8082/master/paymentCancel', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          orderDetailSeqno: orderDetailSeqno,  // 주문 상세 정보 번호
          usedCouponSeqno: usedCouponSeqno,   // 사용된 쿠폰 번호
      ...
      if (response.ok) {
        ...
        alert(data);  // 응답 메시지를 알림으로 표시
      } else {
        const errorData = await response.text();  // 오류 메시지 텍스트 처리
        alert(errorData);
      }
    } catch (error) {
      console.error("주문 취소 처리에 실패했습니다:", error);
      alert("주문 취소 처리 중 오류가 발생했습니다."+orderDetailSeqno );
    }
  };
  ...
  return (
    <div className="d-flex">
      ...
      <div className="container mt-4">
        <h1 className="mb-4">주문 상세 페이지</h1>
              ...
              <table className="table table-striped mt-3">
                ...
                <tbody>
                  <tr>
                    ...
                    <td>{orderInfo.usedPoint}</td>
                    <td>{orderInfo.usedCouponSeqno}</td>
